...
convert_en_to_int <- function(target_col){
  new_col <- ifelse(target_col==eval_data[5], 5,
                    ifelse(target_col==eval_data[4], 4,
                           ifelse(target_col==eval_data[3], 3,
                                  ifelse(target_col==eval_data[2], 2,
                                         ifelse(target_col==eval_data[1], 1, FALSE
                                         )))))
  return(new_col)
}
...
# Converted dataser
int_dataset <- data.frame(
  book = c(convert_en_to_int(new_dataset$book)),
  text = c(convert_en_to_int(new_dataset$text)),
  video = c(convert_en_to_int(new_dataset$video)),
  console = c(convert_en_to_int(new_dataset$console)),
  live = c(convert_en_to_int(new_dataset$live))
)
sum(int_dataset$video==1)
int_dataset$book
int_dataset$video
sum(int_dataset$book)
sum(int_dataset$text)
sum(int_dataset$video)
sum(int_dataset$console)
sum(int_dataset$live)
